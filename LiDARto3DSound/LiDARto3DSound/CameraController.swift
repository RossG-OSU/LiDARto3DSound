...
extension CameraController: AVCapturePhotoCaptureDelegate {
    ...
    func weighted_avg(depthMap: [[Float32]]) -> Float32 {
        let height = depthMap.count
        let width = depthMap[0].count
        let halfWin = (depthMap.count / 2) + (depthMap[0].count / 2)
        var volume = Float32(0)
        var rolling_avg = Float32(0)
        
        for x in 0..<width{
            for y in 0..<height{
                rolling_avg += (depthMap[x][y] * fabsf(Float(((x + y) - halfWin) + 1)))
            }
        }
        
        rolling_avg /= Float(width * height)
        
        volume = 1/rolling_avg
